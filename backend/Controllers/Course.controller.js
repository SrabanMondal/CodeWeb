...
try {
	...
	return res.status(400).json(new ApiResponse(false,"CourseId,Section Title, Questions and TestCases are Required"));
    }
    const course = await Course.findOne({courseid:courseid});
    if(!course){
        return res.status(404).json(new ApiResponse(false,"Course Not Found"));
    ...
    await course.save();
    res.status(200).json(new ApiResponse(true,"Section is Added Successfully"));
} catch (error) {
  res.status(500).json(new ApiError(false, error.message));	
}
}
...
try {
  ...
  if(!title||!description||!options||!courseid){
	return res.status(400).json(new ApiResponse(false,"Title,Description Options and CourseId are required"));
  }
  const course = await Course.findOne({courseid:courseid});
  if(!course){
  ...
  await course.save();
  return res.status(200).json(new ApiResponse(true,"Mcq Added Successfully"));
} catch (error) {
  res.status(500).json(new ApiError(false,error.message));
}
}
